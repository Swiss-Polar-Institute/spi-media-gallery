...
class SpiS3Utils(object):
    def __init__(self, bucket_name: str) -> None:
        if bucket_name not in settings.BUCKETS_CONFIGURATION:
            raise ValueError(
                "Bucket name {} not found. Possible bucket names: {}".format(
                    bucket_name, ", ".join(settings.BUCKETS_CONFIGURATION.keys())
                )
            )
    ...
    def get_presigned_link(
        ...
        params = {
            "Bucket": self._bucket_configuration["name"],
            "Key": key,
    ...
    def get_presigned_download_link(
        self, key: str, filename: Optional[str] = None
    ) -> str:
        if filename is None:
    ...
    def list_files(
        self, prefix: str, only_from_extensions: Optional[List[str]] = None
    ) -> Set[str]:
    ...
    def delete(self, object_storage_key: str):
        file = self.resource().Object(
            self._bucket_configuration["name"], object_storage_key
        )
        return file.delete()
