...
class ProgressReport:
    def __init__(
        ...
        frequency_of_reports: int = 1,
        steps_are_bytes: bool = False,
    ):
        ...
        progress_print(
            "Progress Report - initialized - Total {}:".format(
                self._steps_to_human_readable(total_steps)
            )
        )
    ...
    def print_report_if_needed(self) -> None:
        if (time.time() - self._last_printed_report) > self._frequency_of_reports:
            ...
            total_expected_time = (
                self._total_steps * elapsed_time
            ) / self._current_step
            ...
            progress_print(
                "Processed {} of {}. Elapsed time: {}. Remaining time: {}. ETA: {}.".format(
                    ...
                    datetime.datetime.fromtimestamp(eta)
            ...
            progress_print(
                "Speed: {}/minute Percentage: {:.2f}%".format(
                    self._steps_to_human_readable(
                        speed_per_minute, format_output="{:.2f}"
                    ),
                    percentage_complete,
                )
            )
