...
from ..models import File, Medium, MediumResized, SpiS3Utils
...
class ResizeMediaTest(TestCase):
    ...
    def setUp(self):
        ...
        spi_s3_utils_resource_original.create_bucket(
            Bucket="spi-media-gallery-original"
        )

        self._spi_s3_utils_processed = SpiS3Utils("processed")
        spi_s3_utils_resource_processed = self._spi_s3_utils_processed.resource()
        spi_s3_utils_resource_processed.create_bucket(
            Bucket="spi-media-gallery-processed"
        )
