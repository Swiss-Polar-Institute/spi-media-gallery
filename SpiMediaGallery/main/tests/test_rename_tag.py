...
from ..models import Tag, TagName, TagRenamed
...
class RenameTagTest(TestCase):
    ...
    def test_rename_tag_destination_tag_does_not_exist(self):
        ...
        old_media_ids = set(
            search_for_tag_name_ids([old_id])[1].values_list("id", flat=True)
        )
        ...
        new_media_ids = set(
            search_for_tag_name_ids([new_id])[1].values_list("id", flat=True)
        )
        ...
        self.assertEqual(
            TagRenamed.objects.filter(old_name=old_tag, new_name=new_tag).count(), 1
        )
    ...
    def test_rename_tag_destination_tag_does_exist(self):
        ...
        old_media_ids_before_renaming = set(
            search_for_tag_name_ids([old_id])[1].values_list("id", flat=True)
        )

        # Get the media tagged with the new tag name before renaming
        new_media_ids_before_renaming = set(
            search_for_tag_name_ids([new_id])[1].values_list("id", flat=True)
        )

        # Get the media that are the union of old and new tags (the media that are already tagged by both)
        media_tagged_both_tags_before = old_media_ids_before_renaming.union(
            new_media_ids_before_renaming
        )
        ...
        # Check new tag name does exist in the database after the renaming
        new_tag_name_count = Tag.objects.filter(
            name__name=new_tag, importer=Tag.RENAMED
        ).count()
        self.assertEqual(new_tag_name_count, 1)

        # Get the media tagged with the new tag name after renaming
        new_media_ids_after_renaming = set(
            search_for_tag_name_ids([new_id])[1].values_list("id", flat=True)
        )
    ...
    def test_rename_tag_to_different_case(self):
        """Test the case where the new tag is the same but with a different case and
        the lower case version does exist in the database."""
        ...
        old_tag_name_count = self._tag_name_objects_filter_case_sensitive_count(
            name=old_tag
        )
        self.assertEqual(old_tag_name_count, 1)

        # Check that the new name does not exist in the database before renaming
        new_tag_name_count = self._tag_name_objects_filter_case_sensitive_count(
            name=new_tag
        )
        self.assertEqual(new_tag_name_count, 0)
    ...
    def test_rename_tag_different_case_tag_already_exists(self):
        if settings.FORCE_SQLITE3_DATABASE:
            self.skipTest(
                "This test relies on Mysql behaviour for index and case sensitivity"
            )

        """Test the case where the new tag is different but already exists
        in the database with a letter in a different case."""
        ...
        new_tag_name_count_case_sensitive = (
            self._tag_name_objects_filter_case_sensitive_count(name="Aerial")
        )
