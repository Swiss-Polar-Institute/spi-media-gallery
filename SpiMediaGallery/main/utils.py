...
from .models import File, Medium, Tag, TagName
...
def resize_video(input_file_path: str, width: int) -> Optional[str]:
    ...
    with open(os.devnull, "w") as devnull:
        ...
        command = [
            ...
            "-auto-alt-ref",
            "0",
            # Fixes error: 'Transparency encoding with auto_alt_ref does not work',
            # 'Error initializing output stream 0:0 -- Error while opening encoder for output stream #0:0 -
            # maybe incorrect parameters such as bit_rate, rate, width or height'
            "-max_muxing_queue_size",
            "4096",  # Fixes error: 'Too many packets buffered for output stream 0:1.'
...
def filename_for_resized_medium(
    medium_id: int, photo_resize_label: str, extension: str
) -> str:
...
def get_medium_information(image_filepath: str) -> Dict[str, str]:
    ...
    if date_field is not None and exif[date_field] != "0000:00:00 00:00:00":
        ...
        for possible_format in possible_formats:
            try:
                datetime_processed = datetime.datetime.strptime(
                    datetime_original, possible_format
                )
            ...
            raise ValueError(
                "Cannot convert string to date: _{}_".format(datetime_original)
            )
...
def link_for_medium(file: File, content_disposition: str, filename: str) -> str:
    ...
    if settings.PROXY_TO_OBJECT_STORAGE:
        ...
        return "{}?{}".format(
            reverse("get_file", args=[file.bucket, file.md5]), urllib.parse.urlencode(d)
        )
    else:
        bucket = SpiS3Utils(file.bucket_name())

        return bucket.get_presigned_link(
            file.object_storage_key, content_type, content_disposition, filename
        )
