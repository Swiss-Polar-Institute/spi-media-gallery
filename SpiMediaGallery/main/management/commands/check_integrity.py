...
from ...models import File, Medium, MediumResized, Tag, TagName


class Command(BaseCommand):
    help = (
        "Check integrity: unused tags, unused files in the object storage, files used in the DB"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "elements",
            type=str,
            choices=["files", "tags", "xmp_files", "everything"],
            help="",
        )
        ...
        parser.add_argument(
            "--bucket",
            type=str,
            choices=["original", "processed", "imported"],
...
class CheckOrphanes:
    def __init__(self, elements, bucket_to_check):
        ...
        self._bucket_utils = {
            "original": spi_s3_utils.SpiS3Utils("original"),
        ...
        self._files_in_bucket: Dict[str, Dict[frozenset, set]] = {
            "original": {},
            "processed": {},
        }
    ...
    def check_xmp_files(self):
        ...
        for key in all_keys:
            if key.endswith(".xmp"):
                ...
                if (
                    medium_file_extension in recognised_file_extensions
                    and medium_file not in all_keys
                ):
        ...
        print(
            "* XMP files for a recognized extension that does not have a Medium file:"
        )
    ...
    def _get_files_in_bucket(self, bucket_name, extensions_filter=frozenset()):
        ...
        if (
            self._files_in_bucket[bucket_name] is None
            or extensions_filter not in self._files_in_bucket[bucket_name]
        ):
            print(
                "Collecting files from {} extension filter {}...".format(
                    bucket_name, extensions_filter
                )
            )
            ...
            self._files_in_bucket[bucket_name][extensions_filter] = self._bucket_utils[
                bucket_name
            ].list_files("", only_from_extensions=extensions_filter_or_none)
            print(
                "Number of files collected: {}".format(
                    len(self._files_in_bucket[bucket_name][extensions_filter])
                )
            )
    ...
    def check_orphaned_files(self):
        ...
        if self._bucket_to_check is None or self._bucket_to_check == "original":
            files_bucket_original = self._get_files_in_bucket(
                "original", valid_media_extensions
            )
        ...
        if self._bucket_to_check is None or self._bucket_to_check == "processed":
            files_bucket_processed = self._get_files_in_bucket(
                "processed", valid_media_extensions
            )
        ...
        medium_without_files = self._database_media(
            Medium.objects.filter(file__isnull=True)
        )
