...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--prefix", type=str, default="", help="Prefix to filter for"
        )
...
class ExportMedia(object):
    ...
    def run(self):
        if os.path.exists(self._output_directory):
            raise CommandError(
                "Output directory {} should not exist".format(self._output_directory)
            )
        ...
        media = MediumForView.objects.filter(
            file__object_storage_key__startswith=self._prefix
        )
        ...
        for medium in media:
            ...
            object_storage_key_relative_directory = (
                medium.file.object_storage_key.lstrip("/")
            )
            output_file = os.path.join(
                self._output_directory, object_storage_key_relative_directory
            )

            self._resizes_bucket.download_file(
